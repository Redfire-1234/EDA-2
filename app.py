...
def generate_data_quality_report():
    """Generate comprehensive data quality PDF report"""
    valid, msg = validate_dataframe()
    if not valid:
        return None, msg
    ...
    try:
        ...
        df = app_state.current_df
        filename = f"data_quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        if missing.sum() > 0:
            ...
            for col, count in missing[missing > 0].items():
                pct = (count / len(df)) * 100
                missing_data.append([col, f"{count:,}", f"{pct:.2f}%"])
            
            t = Table(missing_data, colWidths=[2.5*inch, 1.5*inch, 1.5*inch])
            t.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        story.append(Paragraph("Column Details", styles['Heading2']))
        
        col_data = [['Column', 'Type', 'Unique', 'Non-Null']]
        ...
        t = Table(col_data, colWidths=[2*inch, 1.5*inch, 1*inch, 1*inch])
        t.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        if len(df.columns) > 20:
            story.append(Spacer(1, 0.2*inch))
            story.append(Paragraph(f"... and {len(df.columns) - 20} more columns", styles['Normal']))
        
        # Build PDF
        doc.build(story)
        
        return filename, f"✓ Data quality report generated: {filename}"
...
def generate_eda_report():
    ...
    try:
        ...
        if len(numeric_cols) > 0:
            ...
            # Create table
            table_data = [['Statistic'] + list(numeric_cols)]
            for idx in desc_stats.index:
                row = [idx] + [f"{val:.2f}" if not pd.isna(val) else "N/A" 
                              for val in desc_stats.loc[idx]]
                table_data.append(row)
            
            col_widths = [1.2*inch] + [1.1*inch] * len(numeric_cols)
        ...
        if len(numeric_df.columns) >= 2:
            ...
            for col1, col2, corr_val in corr_pairs[:10]:
                corr_data.append([col1[:20], col2[:20], f"{corr_val:.3f}"])
            
            t = Table(corr_data, colWidths=[2*inch, 2*inch, 1.5*inch])
            t.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        return filename, f"✓ EDA report generated: {filename}"
...
def generate_model_report():
    ...
    try:
        ...
        model_info = [
            ['Parameter', 'Value'],
            ['Model Type', str(type(app_state.trained_model).__name__)],
            ['Task Type', app_state.model_type],
            ['Training Samples', f"{len(app_state.X_train):,}"],
            ['Test Samples', f"{len(app_state.X_test):,}"],
            ['Features', f"{app_state.X_train.shape[1]}"]
        ]
        
        t = Table(model_info, colWidths=[3*inch, 3*inch])
        t.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        if app_state.model_type == "Classification":
            ...
            metrics_data = [
                ...
                ['Accuracy', 
                 f"{accuracy_score(app_state.y_train, y_pred_train):.4f}",
                 f"{accuracy_score(app_state.y_test, y_pred_test):.4f}"],
                ['Precision', 
                 f"{precision_score(app_state.y_train, y_pred_train, average='weighted', zero_division=0):.4f}",
                 f"{precision_score(app_state.y_test, y_pred_test, average='weighted', zero_division=0):.4f}"],
                ['Recall', 
                 f"{recall_score(app_state.y_train, y_pred_train, average='weighted', zero_division=0):.4f}",
                 f"{recall_score(app_state.y_test, y_pred_test, average='weighted', zero_division=0):.4f}"],
                ['F1-Score', 
                 f"{f1_score(app_state.y_train, y_pred_train, average='weighted', zero_division=0):.4f}",
                 f"{f1_score(app_state.y_test, y_pred_test, average='weighted', zero_division=0):.4f}"]
            ]
        else:  # Regression
            ...
            metrics_data = [
                ['Metric', 'Training', 'Test'],
                ['MAE', 
                 f"{mean_absolute_error(app_state.y_train, y_pred_train):.4f}",
                 f"{mean_absolute_error(app_state.y_test, y_pred_test):.4f}"],
                ['RMSE', 
                 f"{np.sqrt(mean_squared_error(app_state.y_train, y_pred_train)):.4f}",
                 f"{np.sqrt(mean_squared_error(app_state.y_test, y_pred_test)):.4f}"],
                ['R² Score', 
                 f"{r2_score(app_state.y_train, y_pred_train):.4f}",
                 f"{r2_score(app_state.y_test, y_pred_test):.4f}"]
            ]
        ...
        if app_state.feature_importance_data is not None:
            story.append(Spacer(1, 0.3*inch))
            story.append(Paragraph("Top 10 Important Features", styles['Heading2']))
            
            feat_data = [['Rank', 'Feature', 'Importance']]
            for i, row in app_state.feature_importance_data.head(10).iterrows():
                feat_data.append([
                    str(i+1),
                    row['feature'][:30],
                    f"{row['importance']:.4f}"
            ...
            t = Table(feat_data, colWidths=[0.8*inch, 3*inch, 1.5*inch])
            t.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        return filename, f"✓ Model report generated: {filename}"
        
    except Exception as e:
        return None, f"❌ Error generating report: {str(e)}"

def export_to_excel():
    """Export current dataframe to Excel with multiple sheets"""
    valid, msg = validate_dataframe()
    if not valid:
        return None, msg
    ...
    try:
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            # Main data
            app_state.current_df.to_excel(writer, sheet_name='Data', index=False)
            
            # Summary statistics
            if len(app_state.current_df.select_dtypes(include=[np.number]).columns) > 0:
                app_state.current_df.describe().to_excel(writer, sheet_name='Statistics')
            
            # Missing values
            missing = app_state.current_df.isnull().sum()
            if missing.sum() > 0:
                missing_df = pd.DataFrame({
                    'Column': missing.index,
                    'Missing Count': missing.values,
                    'Percentage': (missing.values / len(app_state.current_df) * 100).round(2)
                })
                missing_df.to_excel(writer, sheet_name='Missing Values', index=False)
            
            # Data types
            dtypes_df = pd.DataFrame({
                'Column': app_state.current_df.columns,
                'Data Type': app_state.current_df.dtypes.values,
                'Unique Values': [app_state.current_df[col].nunique() for col in app_state.current_df.columns]
            })
            dtypes_df.to_excel(writer, sheet_name='Column Info', index=False)
        
        return filename, f"✓ Data exported to Excel: {filename}"
...
# ========================================================================
  ...
  # ========================================================================
    # TAB 6: REPORTING & EXPORT
    # ========================================================================
    
    with gr.Tab("📄 Reporting & Export"):
        gr.Markdown("### Generate Professional Reports and Export Data")
        
        with gr.Row():
            gr.Markdown("""
            **Generate comprehensive reports for your analysis:**
            - **Data Quality Report**: Overview of dataset health, missing values, duplicates
            - **EDA Report**: Statistical summaries, correlations, key insights
            - **Model Performance Report**: Evaluation metrics, feature importance, results
            - **Excel Export**: Multi-sheet workbook with data and statistics
            """)
        
        gr.Markdown("---")
        
        # Data Quality Report
        with gr.Accordion("📊 Data Quality Report", open=True):
            gr.Markdown("Generate a PDF report with dataset overview, missing values analysis, and column details")
            
            dq_btn = gr.Button("📄 Generate Data Quality Report", variant="primary", size="lg")
            dq_file = gr.File(label="Download Report")
            dq_status = gr.Textbox(label="Status", interactive=False)
            
            dq_btn.click(
                fn=generate_data_quality_report,
                outputs=[dq_file, dq_status]
            )
        
        # EDA Report
        with gr.Accordion("📈 EDA Report", open=False):
            gr.Markdown("Generate a PDF report with descriptive statistics, correlations, and key patterns")
            
            eda_btn = gr.Button("📄 Generate EDA Report", variant="primary", size="lg")
            eda_file = gr.File(label="Download Report")
            eda_status = gr.Textbox(label="Status", interactive=False)
            
            eda_btn.click(
                fn=generate_eda_report,
                outputs=[eda_file, eda_status]
            )
        
        # Model Report
        with gr.Accordion("🤖 Model Performance Report", open=False):
            gr.Markdown("Generate a PDF report with model metrics, feature importance, and evaluation results")
            
            model_btn = gr.Button("📄 Generate Model Report", variant="primary", size="lg")
            model_file = gr.File(label="Download Report")
        ...
        # Excel Export
        with gr.Accordion("📊 Excel Export", open=False):
            gr.Markdown("Export data with multiple sheets: main data, statistics, missing values, column info")
            
            excel_btn = gr.Button("📥 Export to Excel", variant="secondary", size="lg")
            excel_file = gr.File(label="Download Excel File")
            excel_status = gr.Textbox(label="Status", interactive=False)
            
            excel_btn.click(
                fn=export_to_excel,
                outputs=[excel_file, excel_status]
            )
        ...
        with gr.Row():
            gr.Markdown("""
            ### 💡 Tips:
            - Generate reports after completing each analysis phase
            - Data Quality Report is best generated immediately after loading data
            - EDA Report should be generated after exploring your data
            - Model Report requires a trained model
            - All reports are timestamped for version control
            """)
...
if __name__ == "__main__":
    print("=" * 80)
    print("🚀 Starting Advanced EDA & ML Application with Reporting")
    print("=" * 80)
    print("\n📊 Features:")
